...
class Mptt
{
     ...
     *  @return void
     */
    function Mptt($projectTypeId = null , $table_name = 'step', $id_column = 'id', $title_column = 'name', $left_column = 'step_left_id', $right_column = 'step_right_id', $parent_column = 'parent_step_id') {
    ...
    function move($source, $target, $position = false) {
        ...
        if (
            ...
            // if target has any children nodes and/or the node needs to be inserted at a specific position
                ...
                // find the target node's child node that exists at the position the new nodes are to be inserted to
                $target_children = array_shift(array_slice($target_children, $position - 1, 1));
    ...
    function _init() {
        ...
        if (!isset($this->lookup)) {
            ...
            $result = mysql_query('
                ...
                FROM
                    ' . $this->properties['table_name'] . '
                WHERE project_type_id = ' . $this->properties['project_type_id'] . '
                ORDER BY
                    ' . $this->properties['left_column'] . '
