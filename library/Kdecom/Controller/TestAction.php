...
class Kdecom_Controller_TestAction extends Zend_Controller_Action {
    ...
    protected function getHiddenElements($string_bulk) {
        ...
        libxml_use_internal_errors(true);
        if (!empty($string_bulk))
            $doc->loadHTML($string_bulk);libxml_use_internal_errors(false);
        $xpath = new DOMXPath($doc);
    ...
    protected function parseCSV($file, $delimiter=',') {
        ...
        foreach ($lines as $line) {
            ...
            foreach ($field_names as $key => $f)
                $_res[$f] = (isset($fields[$key]) ? $fields[$key] : false);
            $res[] = $_res;
        }
        if (!empty($res))
            return $res;else
    ...
    protected function getCookiePath($session_id=false) {
        if ($session_id)
            ...
            $path="/home/tagmyjob/public_html/tmp" . DIRECTORY_SEPARATOR . 'oi.' . $this->getSessionID() . '.cookie';
        return $path;
    }
    ...
    public function init($session_id=false) {
        ...
        $this->settings['cookie_path'] = "/home/tagmyjob/public_html/tmp";
        ...
        if ($this->settings['transport'] == 'curl') {
            ...
            if (strtoupper(substr(PHP_OS, 0, 3)) == 'WIN')
                curl_setopt($this->curl, CURLOPT_CONNECTTIMEOUT, (isset($this->timeout) ? $this->timeout : 5) / 2);
            else
                curl_setopt($this->curl, CURLOPT_TIMEOUT, (isset($this->timeout) ? $this->timeout : 5));
    ...
    protected function get($url, $follow=false, $header=false, $quiet=true, $referer=false, $headers=array()) {
        if ($this->settings['transport'] == 'curl' || true) {
    ...
    protected function getProxy() {
        if (!empty($this->settings['proxies']))
            if (count($this->settings['proxies']) == 1) {
                ...
                return $this->settings['proxies'][array_rand($this->settings['proxies'])];
        return false;
    }
